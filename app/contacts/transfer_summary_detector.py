...
class ContactsManager:
    ...
    def load_contacts_from_excel(self, file_path: Union[str, Path]) -> Tuple[pd.DataFrame, Dict]:
        ...
        try:
            ...
            # Filtrar RUTs válidos y nombres no vacíos
            df_contacts['rut_valido'] = df_contacts['rut'].apply(self.validate_rut)
            df_valid = df_contacts[
                df_contacts['rut_valido'] &
                (df_contacts['nombre'] != '') &
                (df_contacts['nombre'] != 'Nan') &
                (~df_contacts['nombre'].str.lower().isin(['', 'nan', 'null']))
                ].copy()
            ...
            stats = {
                ...
                'sample_contacts': df_unique.head(3)[['rut', 'nombre', 'alias']].to_dict(
                    'records') if not df_unique.empty else []
            }
    ...
    def _detect_bank_transfer_columns(self, df: pd.DataFrame) -> Tuple[Optional[str], Optional[str]]:
        ...
        # Buscar columna de RUT específica para transferencias bancarias
        rut_keywords_exact = [
            'Rut Titular Destino',
            'RUT Titular Destino',
            'rut titular destino',
        ...
        for keyword in rut_keywords_exact:
        ...
        # Buscar columna de nombre específica para transferencias bancarias
        name_keywords_exact = [
        ...
        # Primero buscar coincidencias exactas
        for keyword in name_keywords_exact:
            if keyword in df.columns:
                name_col = keyword
                break
    ...
    def _detect_rut_and_name_columns(self, df: pd.DataFrame) -> Tuple[Optional[str], Optional[str]]:
        """Detecta automáticamente las columnas de RUT y nombre (método genérico de fallback)"""
        rut_col = None
        name_col = None
...
def show_excel_upload(contacts_manager):
    ...
    st.markdown("""
    **📝 Instrucciones:**
    1. Sube el archivo Excel de transferencias bancarias
    2. El sistema detectará automáticamente las columnas "Rut Titular Destino" y "Nombre Titular Destino"
    3. Los contactos válidos se procesarán y guardarán en la base de datos

    **🏦 Columnas esperadas:**
    - `Rut Titular Destino`: Para los RUTs de los destinatarios
    - `Nombre Titular Destino`: Para los nombres completos
    """)

    # File uploader
    uploaded_file = st.file_uploader(
        "Seleccionar archivo Excel de transferencias",
        type=['xlsx', 'xls'],
        help="Archivo Excel del banco con historial de transferencias"
    )
